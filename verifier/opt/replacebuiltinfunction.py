...
class ReplaceBuiltinFunction(NodeVisitor):
    ...
    def visit_Call(self, node : Call):
        ...
        elif isinstance(node.func, IRName) and node.func.name == "randint" and len(node.operands) > 2:
            node.replace_uses_with(RandomSelect(IntegerSet(node.operands[1], node.operands[2])))
